    ...
    <div class="w-full" style="padding: 2rem; padding-left: 18rem;">
        <div class="bg-white p-6 rounded-lg shadow-md flex flex-col gap-3">
            <div class="flex justify-between items-center">
                <h1 class="font-bold" style="font-size: 30px;">Edit Klasifikasi</h1>
            ...
            <form method="POST" action="{{ route('dashboard.klasifikasi.update', ['klasifikasi' => $klasifikasi->id]) }}" class="flex flex-col gap-4">
                @csrf
                @method('PUT')
                    ...
                    <div class="flex w-full gap-4">
                        <div class="flex rounded-lg w-full">
                            ...
                            <input type="text" name="nama"
                                class="py-1.5 sm:py-2 px-3 pe-11 block w-full border-gray-200 rounded-e-lg sm:text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" value="{{ $klasifikasi->nama }}" required>
                        </div>
                    </div>
                </div>
